...
export default function VerifyEmailSentScreen() {
  ...
  const handleResend = useCallback(async () => {
    ...
    try {
      ...
      // silencioso — el mensaje genérico del backend no aporta nada útil
    } finally {
      setResending(false);
    }
  }, [countdown, resending, email, resendVerification, startCountdown]);
      ...
      <View style={styles.content}>
        ...
        <View style={[styles.iconWrap, { backgroundColor: colors.tint + '18' }]}>
          <View style={[styles.iconInner, { backgroundColor: colors.tint + '28' }]}>
            <Ionicons name="mail" size={44} color={colors.tint} />
          </View>
        </View>
        ...
        <ThemedText style={[styles.description, { color: colors.icon }]}>
          Hacé clic en el enlace del email para activar tu cuenta. Si no lo encontrás, revisá la carpeta de spam.
        </ThemedText>
        ...
        <View style={[styles.stepsCard, { backgroundColor: cardBg }]}>
          {[
            ...
            { icon: 'link-outline' as const,         text: 'Hacé clic en el enlace de confirmación' },
            { icon: 'checkmark-circle-outline' as const, text: 'Tu cuenta queda activada' },
          ].map((step, i, arr) => (
            <View key={i}>
        ...
        <TouchableOpacity
          ...
          disabled={!canResend}>
          {resending ? (
            <ThemedText style={[styles.resendText, { color: canResend ? '#fff' : colors.icon }]}>
              Enviando…
            </ThemedText>
          ) : resendDone && countdown > 0 ? (
            <ThemedText style={[styles.resendText, { color: colors.icon }]}>
              Email reenviado · {countdown}s
            </ThemedText>
          ) : countdown > 0 ? (
            <ThemedText style={[styles.resendText, { color: colors.icon }]}>
              Reenviar verificación · {countdown}s
            </ThemedText>
          ) : (
            <ThemedText style={[styles.resendText, { color: '#fff' }]}>
