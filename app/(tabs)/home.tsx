...
const CATEGORIES: CategoryDef[] = [
  ...
  { key: 'history', labelKey: 'homeTab.categories.history', icon: 'time-outline', bg: '#FBE8EF', fg: '#BE185D', filterKind: ['place'], filterCategory: ['patrimonio'] },
...
function FeaturedCard({ trail, large }: { trail: Trail; large?: boolean }) {
  ...
  const onHeartPress = async () => {
    if (!token) {
      redirectToLogin(pathname || '/(tabs)/home');
      return;
    }
...
function HomeContent() {
  ...
  const goToList = (filters?: { filterKind: string[]; filterCategory: string[] }) => {
    if (filters) setPendingListFilters(filters);
    setMode('list');
    router.push('/(tabs)');
  };
